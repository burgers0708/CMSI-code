...
def cam_pruning(A, X, cutoff, prune_only=True, pns=False):
    ...
    # save_path = "/root/TPAMI_test/"
    save_path = os.path.join(os.path.dirname(os.path.dirname((__file__))), 'TPAMI_test')
    ...
    if prune_only:
        ...
        Rpath = os.path.join(os.path.dirname(__file__), 'cam_pruning.R')
        assert(os.path.exists(Rpath))
        dag = launch_R_script(Rpath, arguments, output_function=retrieve_result)
        return dag
    else:
        def retrieve_result():
            A = pd.read_csv(arguments['{PATH_RESULTS}']).values
...
def SCORE(X, eta_G=0.001, eta_H=0.001, cutoff=0.001, normalize_var=False, dispersion="var", pruning = 'CAM', threshold=0.1):
    ...
    # t2 = time.time()
    # print("compute_top_orderTime:", t2-t1)
    if pruning == 'CAM':
        return cam_pruning(full_DAG(top_order), X, cutoff), top_order
        # print("cam_pruningTime:", time.time()-t2)
    elif pruning == 'Stein':
